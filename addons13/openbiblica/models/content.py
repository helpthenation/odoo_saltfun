...
class Contents(models.Model):
    ...
    team_ids = fields.Many2many('res.users', relation='content_team_1_rel', string='colaborators')
    approver_ids = fields.Many2many('res.users', relation='content_approver_1_rel', string='approvers')
...
class SubContents(models.Model):
    ...
    create_id = fields.Many2one('res.users', string='Author', index=True)
    team_ids = fields.Many2many('res.users', relation='subcontent_team_1_rel', string='colaborators')
    approver_ids = fields.Many2many('res.users', relation='subcontent_approver_1_rel', string='approvers')
    create_date = fields.Datetime('Started on', index=True, readonly=True)
    write_date = fields.Datetime('Updated on', index=True, readonly=True)
    active = fields.Boolean(default=True)
    open_project = fields.Boolean(default=True)
