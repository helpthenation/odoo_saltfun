...
class SmsPartner(models.Model):
    ...
    name = fields.Char(string=u'服务商名称', required=True, index=True)
    code = fields.Char(string=u'标识', required=True, index=True)
    image = fields.Binary(string=u"图标", default=_get_default_image)
    app_id = fields.Char(string='AccessKey', required=True)
    app_key = fields.Char(string='AccessSecret', help=u"Secret是用来校验短信发送请求合法性的密码", required=True)
    priority = fields.Integer(string=u'优先级(1-10)', default=5)
    code_length = fields.Integer(string=u'验证码长度', default=6)
    template_ids = fields.One2many(comodel_name='sms.mytemplate', inverse_name='partner_id', string=u'短信模板')
