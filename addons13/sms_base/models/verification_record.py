...
class SmsVerificationRecord(models.Model):
    ...
    template_id = fields.Many2one(comodel_name='sms.mytemplate', string=u'模板', ondelete='cascade', index=True)
...
class SmsSendRecord(models.Model):
    ...
    create_date = fields.Datetime(string=u"创建时间", default=fields.Datetime.now, index=True)
    partner_id = fields.Many2one(comodel_name="sms.partner", string=u"服务商", index=True, ondelete='cascade')
    signature_id = fields.Many2one(comodel_name="sms.signature", string=u"短信签名", ondelete='cascade', index=True)
    template_id = fields.Many2one(comodel_name='sms.mytemplate', string=u'模板', ondelete='cascade', index=True)
    code = fields.Char(string=u"模板代码", index=True)
